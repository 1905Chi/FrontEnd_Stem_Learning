...
import HeartIcon from '../../assets/icons/icons8-heart-50.png';
import HahaIcon from '../../assets/icons/icons8-grinning-squinting-face-50.png';
import LikeIcon from '../../assets/icons/icons8-like-50.png';
import SadIcon from '../../assets/icons/icons8-sad-50.png';
import WowIcon from '../../assets/icons/icons8-wow-50.png';
import AngryIcon from '../../assets/icons/icons8-angry-50.png';
...
function MessageItem({ isOwner, message, stompClient, currentUser, onRetry }) {
	...
	const handleReactIcon = (icon) => {
		...
		if (message.chatRoomId === null || message.chatRoomId === 'null' || message.chatRoomId === undefined) {
			if (message.react !== undefined && message.react !== null) {
				const index = message.react.findIndex((item) => item.reactUser === currentUser?.id);
				if (index !== -1) {
	...
	const handleConfirmAction = async () => {
		...
		if (isRemove) {
			...
			if (message?.chatRoomId && message?.chatRoomId !== 'null') {
				stompClient.send('/app/room-message/' + data?.id, {}, JSON.stringify(data));
			} else {
	...
	return (
		<Space className={classnames(classParent)} align={isOwner ? 'end' : 'start'}>
			{!isOwner && (
				<img
					src={message?.senderAvatarUrl || "https://img.icons8.com/ios/50/user--v1.png"}
					alt="avatar"
