...
function MainCompetition() {
	...
	return (
		<div className="main-competition">
			<img
				src={group && group.avatarUrl !== null && group.avatarUrl !== '' ? group.avatarUrl : anh_logo_1}
				alt="anh"
				className="img-baner"
				style={{ display: group ? 'block' : 'none' }} // Chỉ hiển thị khi group có giá trị
			/>
			<Tabs defaultActiveKey="1" items={items} centered tabBarGutter={16} onChange={(key) => setActiveKey(key)} />
			<ToastContainer />
		</div>
	);
}
