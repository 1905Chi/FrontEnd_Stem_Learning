		...
		if (currentUser && !stompClient) {
			...
			client.connect(
				...
				() => {
					...
					client.subscribe('/user/' + currentUser?.id + '/notification', (e) =>
						setNotification(JSON.parse(e.body))
					);
					client.send('/app/user.addUser', {}, JSON.stringify(data));
	...
	const disconnectWebSocket = (currentUser) => {
		if (stompClient) {
			...
			setTimeout(() => {
				...
				stompClient.send('/app/user.disconnectUser', {}, JSON.stringify(data));
